...
namespace WP2Github_MD;

class Converter {
	/**
	 * @param string $readme plugin readme.txt content
	 * @param string $plugin_slug explicitly set the plugin slug, NULL for autodetect
	 *
	 * @return string
	 */
	public static function convert( $readme, $plugin_slug = null ) {
		...
		if ( preg_match( '|## Screenshots (.*?)## [a-z]+ |ism', $readme, $matches ) ) {
			...
			foreach ( $screenshots as $screenshot ) {
				$screenshot_url = self::find_screen_shots( $i, $plugin );
				if ( $screenshot_url ) {
	 ...
	 */
	private static function find_screen_shots( $number, $plugin_slug ) {
	 ...
	 */
	private static function validate_url( $link ) {
